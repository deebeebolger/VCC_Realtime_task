...
fileout_labels0 = open("../Image_classify/labels_0.dat", 'w')
fileout_labels1 = open("../Image_classify/labels_1.dat", 'w')
fileout_labels2 = open("../Image_classify/labels_2.dat", 'w')
fileout_labels3 = open("../Image_classify/labels_3.dat", 'w')
...
for i, currfile in enumerate(path_contents):  # Range of user.
    ...
    labelIn = dataIn['labels']
    print(f'Loading file {currfile}')
    plot_channels(dataIn['data'], 128, chan, currfile)
    for trl in range(nTrial):  # Trial range
        fileout_data = open(os.path.join("../Image_classify/features_raw.csv"), 'w')  # Created for each trial (video) and each participant.

        currtrl_data = AllData[trl, 0:32,:]
        fband_trl = PSD_calc(currtrl_data, srate)                   # takes in channel-level data for each trial.

        if trl == 0:
            Fbands_alltrls = fband_trl
        else:
            Fbands_alltrls = np.dstack((Fbands_alltrls,fband_trl))  # Append the mean power in the 3 frequency bands for the current trial.

        for ichan, chancurr in enumerate(chan):
            fileout_data.write(chancurr+",")
        fileout_data.write("\n")
        for currdat in range(nTime):
            for ichan2 in range(len(chan)):
                if ichan2 < 32:
                    if ichan2 == 31:
        ...
        os.system('python VCC_create_vector.py')    # Command to execute "VCC_create_vector.py
